...
// Pre-sizes uploaded images before Content Safety scan + blob storage.
public interface ImageResizeService {
    ...
    // Returns a size×size JPEG at 85% quality
    byte[] resize(MultipartFile file, int size);

    // Same as resize(MultipartFile, int) but accepts raw bytes
