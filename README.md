...
a containerised, full-stack group task management platform deployed on 15 Azure PaaS resources — built as a BSc thesis to prove that enterprise-grade security, performance engineering, and operational automation can work on a student budget (~$108–128/month). not a demo. not a prototype. every layer was designed around real cloud patterns, validated by nine automated OWASP attack simulations (41 assertions, 100% pass rate), and cost-engineered so the FREE tier generates exactly $0 variable Azure cost.

users create groups, manage tasks collaboratively, assign five per-group roles, upload files with AI content moderation, review task submissions, leave comments, track presence, send invitations by email, and work within plan-based usage limits. the system includes a four-tier subscription model (FREE / STUDENT / ORGANIZER / TEAM) that atomically gates every cost-generating Azure API call at the database level, an admin back-office, group event audit logs, and an encrypted client-side cache.
...
- **subscriptions** — four tiers with atomic SQL budget enforcement (`UPDATE ... WHERE budget >= cost`) — if the budget is exceeded, the Azure API is never invoked. no distributed locks, no race conditions
...
[[[Image here: high-level system architecture diagram — React SPA → Azure Front Door (CDN + WAF) → two origin paths: (1) Blob Storage for static assets, (2) App Service for /api/* → backend connects to Azure SQL, Redis, Blob Storage, Content Safety, ACS, Key Vault → Container App Jobs for maintenance → all via Managed Identity]]]
...
**frontend** — React 19 with the React Compiler (automatic memoisation), Vite 7, ~70 JSX files across 11 routes and 50+ reusable components. five Context providers in dependency order. dark mode via CSS custom properties with pre-hydration flash prevention.

**database** — Azure SQL (S1, 20 DTU) with 14 JPA entities, 75+ custom `@Query` methods, seven Flyway migrations. every association-loading query uses `LEFT JOIN FETCH` (1 SQL statement instead of 201 for N+1 problems). H2 compatibility mode for zero-cost local development.
...
| caching | Azure Cache for Redis (Enterprise B0) — 4 services with disjoint key namespaces |
| storage | Azure Blob Storage (Standard_LRS) — 4 app containers + 3 frontend containers |
...
| testing | k6 — 9 OWASP attack scripts (41 assertions) + 2 stress tests |
...
**data protection** — Managed Identity is the single credential for SQL, Blob, and Key Vault (zero passwords in config). 13 Key Vault secrets fetched once at startup. six security headers enforced at the CDN edge (HSTS, CSP, X-Frame-Options, X-Content-Type-Options, Referrer-Policy, Permissions-Policy).
...
two stress tests validate the B1 tier under concurrent load: polling load (30 VUs, 2s intervals) and presence storm (50 VUs, 30s intervals). these run against a dedicated arena environment deployed from the same Bicep template — production defences intact, no risk to real users.
...
every cost-generating API call (upload, download, email, image scan) is gated by an atomic SQL check before the Azure API is invoked:
...
| ACR | Basic | $5 |
| everything else (Key Vault, ACS, Content Safety, Blob, monitoring) | various | ~$2–17 |
...
*15 Azure resources · 10 Maven modules · 75+ REST endpoints · 14 JPA entities · 7 Flyway migrations · 4 subscription tiers · 9 OWASP attack scripts · 41 security assertions · $108–128/month*
