...
- **Post-v1.0 versioning approach:** Once v1.0.0 is tagged, all modules adopt a unified lock-step version managed by a root `VERSION` file, Maven `${revision}` property with `flatten-maven-plugin`, and npm `version` in `package.json`. Releases are automated via `commit-and-tag-version`, which scans git history **from the last tag forward only** (pre-v1.0 commits are never touched), determines the SemVer bump from Conventional Commit prefixes (`feat:` → minor, `fix:` → patch, `BREAKING CHANGE` → major), generates `CHANGELOG.md`, and creates the release commit + tag. Commit scopes (e.g., `feat(backend):`, `fix(frontend):`) identify which modules were affected in each release.
